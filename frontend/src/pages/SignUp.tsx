...
export default function SignUpPage() {
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.error(
        err.errors?.[0]?.message ?? "Account creation failed. Try again.",
      );
    } finally {
      setLoading(false);
    }
  ...
  const handleVerify = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.status === "complete") {
        ...
        toast.success("Welcome to The Loop", "Your account is ready.");
        navigate("/");
      } else {
      ...
      toast.error(
        err.errors?.[0]?.message ?? "Code verification failed. Try again.",
      );
    } finally {
      setLoading(false);
    }
